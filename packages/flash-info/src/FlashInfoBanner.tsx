...
function getFlashInfoKey({ id, modifiedDate }: Pick<FlashInfoItem, "id" | "modifiedDate">) {
...
export default function FlashInfoBanner({ flashInfos, color = "primary" }: FlashInfoBannerProps) {
  ...
  const [closedFlashInfoKeys, setClosedFlashInfoKeys] = useLocalStorage<string[]>(
    FLASH_INFO_CLOSED_KEY,
    [],
  );

  const closeAll = () => {
    setClosedFlashInfoKeys(flashInfos.map((f) => getFlashInfoKey(f)));
    setIsClosed(true);
  };
