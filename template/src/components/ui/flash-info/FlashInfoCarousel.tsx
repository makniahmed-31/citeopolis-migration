...
export default function FlashInfoCarousel({
  flashInfos,
}: FlashInfoCarouselProps) {
  ...
  useEffect(() => {
    ...
    const controlsElement = document.querySelector<HTMLElement>(
      `.${styles.swiperControls}`,
    );
    const closeButton = swiperElement
      ?.closest("article")
    ...
    const handleFocusOut: EventListener = (e) => {
      ...
      if (
        nextTarget &&
        (swiperElement.contains(nextTarget) ||
          controlsElement.contains(nextTarget))
      ) {
        return;
      }
  ...
  return (
    <>
      <div
        ...
        aria-label="Contrôle du carrousel"
      >
        ...
        >
          <i
            className={clsx("fa", isPlaying ? "fa-pause" : "fa-play")}
            aria-hidden="true"
          />
        </button>
      </div>
